...
from motion import CompositePrimitive, MotionPrimitive, TerminateOnGroundWrapper, Wait, Walk
...
class RRTNode:
    ...
    reachable: Dict[int, float] = (
        None  # Maps segment hash of reachable segments to distance
    )
...
class RRTGraph:
    def __init__(self, segments: list[Segment] = None):
        ...
        self.floor_segments = (
            {}
            if segments is None
            else {hash(seg): seg for seg in segments if seg.type == "floor"}
        )
        self.nodes_for_floor = {floor:[] for floor in self.floor_segments.keys()}
    ...
    def add_node(
        ...
        if segment is not None:
            # if segment not in self.nodes_for_floor:
            #     self.nodes_for_floor[segment] = []
            bisect.insort(self.nodes_for_floor[segment], node_id, key=lambda nid: self.nodes[nid].sim_state.x_pos)
    ...
    def update_segment_reachability(self, terrain):
        ...
        for node_id, node in self.nodes.items():
            if node.segment is not None:
                node.reachable[node.segment] = (
                    0.0  # Zero distance to the segment it's on
                )
        ...
        while queue:
            ...
            for parent_id in reverse_graph[node_id]:
                ...
                for segment_hash, distance in node.reachable.items():
                    ...
                    if (
                        segment_hash not in parent.reachable
                        or new_distance < parent.reachable[segment_hash]
                    ):
    ...
    def connect_same_segment_nodes(
        self, floor_segments: list[Segment], terrain: Terrain, run_speed: float = 8.3
    ):
        """
        ...
        for segment_id, nodes in segment_nodes.items():
            ...
            for i in range(len(nodes) - 1):
                ...
                self.graph.add_edge(
                    node_id1,
                    node_id2,
                    control=Walk(cost, x_displacement > 0),  # No specific control, just direct movement
                    duration=cost,
                    weight=cost,
                )
                self.graph.add_edge(
                    node_id2, node_id1, control=Walk(cost, x_displacement < 0), duration=cost, weight=cost
                )
    ...
    def prune_non_essential_nodes(self, essential_node_ids: set[int]):
        ...
        print(
            f"Pruned graph from {len(self.nodes) + len(non_essential_nodes)} to {len(self.nodes)} nodes"
        )

    def find_path(
        self,
        start_state: np.ndarray | HeroControllerStates,
        goal_state: np.ndarray | HeroControllerStates,
        k_nearest=1,
    ):
        """
        ...
        print(
            f"finding path from {start_state} to {goal_state} via {appx_start_pos} and {appx_goal_pos}"
        )
    ...
    def sample_connections_sync(
        self,
        terrain: Terrain,
        num_samples: int = 1000,
        samples_per_segment: int = 5,
        max_edge_duration: float = 0.8,
        prune_nodes: bool = True,
        connect_same_segment: bool = True,
    ...
    async def sample_connections_async(
        ...
        print(f"Sampling RRT graph for terrain with {num_samples} samples.")
        i = 0
        def shuffled_floor_iter():
            vals = list(self._priv_graph.floor_segments.values())
            np.random.shuffle(vals)
            return iter(vals)
        floors_to_sample = shuffled_floor_iter()
        # print('shuffling floors', list(floors_to_sample))
        floors_to_sample = shuffled_floor_iter()
        while i < num_samples:
            j = len(self._priv_graph.nodes)
            if j % publish_interval == 0 and j > 0:
                print(f"{j} samples done. Publishing intermediate results...")
                self._publish_pruned_graph(prune_nodes, connect_same_segment)
                # Yield to event loop periodically
                await asyncio.sleep(0)
            # Yield to event loop occasionally to prevent blocking
            if i % 50 == 0:
                # print('sampling rrt', i, j)
                await asyncio.sleep(0)
            i += 1

            try:
                floor = next(floors_to_sample)

            except StopIteration:
                floors_to_sample = shuffled_floor_iter()
                floor = next(floors_to_sample)

            if floor is None or len(self._priv_graph.nodes_for_floor[hash(floor)]) == 0:
                continue
            # self._sample_one_node(terrain, i, num_samples, max_edge_duration)
            self._sample_one_node_from_floor(terrain, floor, j)

            # Final update
            # floor_idx = (floor_idx + 1) % len(self.floor_segments.keys())
    ...
    def _initialize_floor_nodes(self, terrain: Terrain, samples_per_segment: int):
        ...
        for segment in self._priv_graph.floor_segments.values():
            ...
            padding = 0.6
            dist_between = 3
            min_x = min(segment.x_min + padding, segment.x_max)
            max_x = max(segment.x_max - padding, segment.x_min)
            num_segs = max(int(abs(max_x - min_x) / dist_between), 2)
            x_coords = np.linspace(min_x, max_x, num_segs)
            y_coords = (
                np.ones_like(x_coords) * segment.y_min
            )  # Assuming floor segments are flat
    ...
    def _sample_one_node(
        self, terrain: Terrain, i: int, num_samples: int, max_edge_duration: float
    ):
        ...
        nearest_id = np.random.choice(
            self._priv_graph.get_k_nearest_nodes(state_rand, k=5)
        )
        ...
        control = TerminateOnGroundWrapper(control)
        full_state = parent_node.sim_state

        floor_segment = None
        # Forward simulation
        action_duration = 0
        control.reset_state(full_state)
        while not control.is_terminated and action_duration < 8:
            action, terminated = control.get_action(full_state)
            full_state = forward_dynamics(
                full_state, terrain, action, 1 / FIXED_UPDATE_FRAMERATE
            )
            action_duration += 1 / FIXED_UPDATE_FRAMERATE
        ...
        #         floor_segment = hash(get_ground_segment(full_state, terrain))
        #         reached_ground = True
        #         if floor_segment != orig_floor:
        #             control = control.trim_duration(action_duration)
        #             break
        # if not reached_ground:
        #     fall_duration = 0
        #     for action in PlayerInput.get_action_iterator(
        #         Wait(4).control_sequence(), FIXED_UPDATE_FRAMERATE
        #     ):
        #         full_state = forward_dynamics(
        #             full_state, terrain, action, 1 / FIXED_UPDATE_FRAMERATE
        #         )
        #         fall_duration += 1 / FIXED_UPDATE_FRAMERATE
        #         if full_state.onGround:
        #             floor_segment = hash(get_ground_segment(full_state, terrain))
        #             break
        #     control = CompositePrimitive([control, Wait(fall_duration)])
    ...
    def _sample_one_node_from_floor(self, terrain: Terrain, floor: Segment, num_already_sampled: int):
        def sample_end_biased_index(n, alpha=0.5):
            """
            Sample an index in [0, n) biased toward 0 and n-1.
            α controls how strong the bias is: smaller α => stronger end-bias.
            """
            # draw x in [0,1] with U-shape
            x = np.random.beta(alpha, alpha)
            return min(int(x * n), n-1)
        floor_nodes = self._priv_graph.nodes_for_floor[hash(floor)]
        sample_idx = sample_end_biased_index(len(floor_nodes))
        parent_node_id = floor_nodes[sample_idx]
        parent_node = self._priv_graph.nodes[parent_node_id]

        # Sample control and simulate
        def duration_schedule(i):
            return np.random.uniform(0.3, 2)
        duration = duration_schedule(num_already_sampled)
        control = MotionPrimitive.sample(duration)
        control = TerminateOnGroundWrapper(control)
        full_state = parent_node.sim_state
    ...
    def _publish_pruned_graph(self, prune_nodes: bool, connect_same_segment: bool):
        ...
        working_graph.bounds_min = (
            self._priv_graph.bounds_min.copy()
            if self._priv_graph.bounds_min is not None
            else None
        )
        working_graph.bounds_max = (
            self._priv_graph.bounds_max.copy()
            if self._priv_graph.bounds_max is not None
            else None
        )
        ...
        if connect_same_segment:
            working_graph.connect_same_segment_nodes(
                list(working_graph.floor_segments.values()), self._terrain
            )
...
def is_within_scene_bounds(state: np.ndarray, terrain: Terrain) -> bool:
    ...
    """
    return (
        0 <= state[0] <= terrain.scene_width and 0 <= state[1] <= terrain.scene_height
    )
...
def visualize_graph(graph: RRTGraph, terrain: Terrain, show_edges=True):
        ...
        if show_edges:
            ...
            for u, v in graph.graph.edges():
                ...
                ax.annotate(
                    ...
                    arrowprops=dict(arrowstyle="->", lw=2, color=col, alpha=0.3),
                )
                if (
                    node_u.segment is not None
                    and node_v.segment is not None
                    and node_u.segment != node_v.segment
                ):
                    assert keep
    ...
    handles = [
        plt.Line2D([0], [0], color=c, label=t)
        for t, c in [
            ("Floor", "gray"),
            ("Wall", "darkgray"),
            ("Ceiling", "lightgray"),
...
def visualize_floor_connections(graph: RRTGraph, terrain: Terrain):
    ...
    if hasattr(graph, "floor_connection_graph"):
        ...
        costs = [
            data["weight"]
            for _, _, data in graph.floor_connection_graph.edges(data=True)
        ]
        ...
        if costs:
            ...
            # Draw each edge
            for from_hash, to_hash, data in graph.floor_connection_graph.edges(
                data=True
            ):
